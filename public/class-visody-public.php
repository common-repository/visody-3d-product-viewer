...
class Visody_Public
{
	...
	public function get_inline_styles()
	{
		...
		if ($btn_hover_bg = get_option('visody_button_hover_bg_color')) {
			...
			$style .= ' .vsd-model-viewer button:hover, .vsd-model-viewer button:focus { background-color: ' . esc_attr($btn_hover_bg) . '; }';
		}
		if ($btn_hover_text = get_option('visody_button_hover_text_color')) {
			$style .= ' .vsd-model-viewer-control-buttons button:hover, .vsd-model-viewer-control-buttons button:focus,';
			$style .= ' .vsd-model-viewer button:hover, .vsd-model-viewer button:focus { color: ' . esc_attr($btn_hover_text) . '; }';
		}
	...
	public function get_inline_script()
	{
		...
		$visody_gallery_selector = '.woocommerce-product-gallery';
		if (get_option('visody_class_gallery')) {
			$visody_gallery_selector = esc_attr(get_option('visody_class_gallery'));
		}

		$visody_gallery_wrapper_selector = '.woocommerce-product-gallery__wrapper';
		if (get_option('visody_class_gallery_wrapper')) {
			$visody_gallery_wrapper_selector = esc_attr(get_option('visody_class_gallery_wrapper'));
		}

		$visody_gallery_item_selector = '.woocommerce-product-gallery__image';
		if (get_option('visody_class_gallery_item')) {
			$visody_gallery_item_selector = esc_attr(get_option('visody_class_gallery_item'));
		}

		$visody_gallery_item_active_selector = '.woocommerce-product-gallery__image.flex-active-slide';
		if (get_option('visody_class_gallery_item_active')) {
			$visody_gallery_item_active_selector = esc_attr(get_option('visody_class_gallery_item_active'));
		}

		$visody_gallery_thumb_selector = '.flex-control-thumbs li';
		if (get_option('visody_class_gallery_thumbnail_item')) {
			$visody_gallery_thumb_selector = esc_attr(get_option('visody_class_gallery_thumbnail_item'));
		}

		$visody_gallery_trigger_selector = '.woocommerce-product-gallery__trigger';
		if (get_option('visody_class_gallery_trigger')) {
			$visody_gallery_trigger_selector = esc_attr(get_option('visody_class_gallery_trigger'));
		}

		$visody_check_interval = '200';
		if (get_option('visody_check_gallery_interval')) {
			$visody_check_interval = esc_attr(get_option('visody_check_gallery_interval'));
		}
		...
		var visody_check_interval = ' . esc_attr($visody_check_interval) . ';
		';
	}
